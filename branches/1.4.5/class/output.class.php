...
abstract class output {
     ...
     * @param string $url
     */
    static public function Boink($url='./', $messager='') {
        if ($messager != '')
            self::Messager($messager);

        DataEngine::sql_log();
        // Remplacement de variable...
        $url = str_replace('%ROOT_URL%', ROOT_URL, $url);
        $url = str_replace('%INCLUDE_URL%', INCLUDE_URL, $url);
