...
function move(e) {
    if(bullevisible) {  // Si la bulle est visible, on calcul en temps reel sa position ideale
        ...
        if ( cw < ($("curseur").clientWidth+Mousex+spacingx+5)) {
            $("curseur").style.left=(cw-spacingx-$("curseur").clientWidth)+"px";
            $("curseur").style.top=Mousey+spacingy+"px";
        } else {
            $("curseur").style.left=spacingx+Mousex+"px";
            $("curseur").style.top=spacingy+Mousey+"px";
        }
    }
}
