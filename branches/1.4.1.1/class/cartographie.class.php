...
class cartographie {
    ...
    function add_vortex($coordsin, $coordsout, $infos='', $nom='', $note='') {
        ...
        if (mysql_num_rows($mysql_result) > 0) {
            ...
            if($ligne['ID'] > 0) {
                DataEngine::sql('UPDATE SQL_PREFIX_Coordonnee SET `INACTIF`=0,`USER`=\''.$nom.'\',`INFOS`=\''.$infos.'\',`NOTE`=\''.$note.'\' WHERE TYPE=1 AND (POSIN=\''.$INidfixe.'\' AND POSOUT=\''.$OUTidfixe.'\')');
                if (mysql_affected_rows() > 0)
                    $this->AddInfo('Le vortex '.$coordsin.' vers '.$coordsout.' a été réactivée/modifié');
                else
...
//    $types = '0,3,5,6'; // Type pris en charge...
    ...
    public function add_planet_asteroid($coords, $ress_val, $type, $note="") {
        ...
        if ($type==2 && !DataEngine::CheckPerms(CXX_CARTOGRAPHIE_PLAYERS))
            return $this->AddErreur('Permissions manquante');
        if ($type==4 && !DataEngine::CheckPerms(CXX_CARTOGRAPHIE_ASTEROID))
            return $this->AddErreur('Permissions manquante');
        
        if (($warn=$this->FormatId(trim($coords), $uni, $sys)) !="")
            return array('',$warn.' pour '.(($type==2)? 'la planète': 'l\'astéroïde') );
        ...
        foreach ($ress_val as $id => $value) {
            ...
            if(!$this->Ressources_Check_Value($value, $type)) {
                $this->AddErreur('Format de la valeur de la ressource '.$Ressource[$id]['Nom'].' incorrecte ('.$value.'), autorisé : (p)eu,(n)ormal,(b)eaucoup,xx,xx%');
            }
        ...
        if ($do_update) {
            $updated = 0;
            $query = "UPDATE `SQL_PREFIX_Coordonnee` SET `NOTE`='$qnote' WHERE `ID`=$do_update";
            DataEngine::sql($query);
    ...
    public function __construct() {
        ...
        $this->allys = Config::GetEmpireAllys();
        $this->wars  = Config::GetEmpireEnnemy();
        if (!is_array($this->allys)) $this->allys = array($this->allys);
        if (!is_array($this->wars)) $this->wars = array($this->wars);
