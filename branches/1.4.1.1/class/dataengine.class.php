...
class DataEngine {
    ...
    static public function sql_log() {
        ...
        if (IN_DEV) {
            if (count(self::$sqls)>1) {
                $time = 0; foreach(self::$sqls as $v) $time += $v[0];
    ...
    static public function _tpl_defined($tpl) { return isset(self::$tpls[$tpl]); }
...
ead;
    ...
    static public function NoPermsAndDie() {
        self::ErrorAndDie('Permission minimale manquante ou option désactivée.');
    }
    /**
     * Supprimer un joueur ?
    ...
    static private function Perms() {

        if (!defined('CXX_CARTOGRAPHIE'))          define('CXX_CARTOGRAPHIE', AXX_MEMBER);
        if (!defined('CXX_CARTOGRAPHIE_SEARCH'))   define('CXX_CARTOGRAPHIE_SEARCH', AXX_MEMBER);
        if (!defined('CXX_CARTOGRAPHIE_PLAYERS'))  define('CXX_CARTOGRAPHIE_PLAYERS', AXX_MEMBER);
        if (!defined('CXX_CARTOGRAPHIE_ASTEROID')) define('CXX_CARTOGRAPHIE_ASTEROID', AXX_MEMBER);

        if (!defined('CXX_CARTOGRAPHIE_GREASE'))   define('CXX_CARTOGRAPHIE_GREASE', AXX_MEMBER);

        if (!defined('CXX_CARTE'))                define('CXX_CARTE', AXX_MEMBER);
        if (!defined('CXX_CARTE_JOUEUR'))         define('CXX_CARTE_JOUEUR', AXX_MEMBER);
        if (!defined('CXX_CARTE_SHOWEMPIRE'))     define('CXX_CARTE_SHOWEMPIRE', AXX_MEMBER);
        if (!defined('CXX_CARTE_SEARCH'))         define('CXX_CARTE_SEARCH', AXX_MEMBER);

        if (!defined('CXX_PERSO'))                define('CXX_PERSO', AXX_MEMBER);
        // Désactivé, pas besoin de compte pour cette page. voir 'recherche.php' pour ré-activer.
        if (!defined('CXX_PERSO_RESEARCH'))       define('CXX_PERSO_RESEARCH', AXX_GUEST);
        if (!defined('CXX_PERSO_OWNUNIVERSE'))    define('CXX_PERSO_OWNUNIVERSE', AXX_MEMBER);


        if (!defined('CXX_MEMBRES_HIERARCHIE'))   define('CXX_MEMBRES_HIERARCHIE', AXX_MEMBER);
        if (!defined('CXX_MEMBRES_NEW'))          define('CXX_MEMBRES_NEW', AXX_ADMIN); // inclus les grades...
        if (!defined('CXX_MEMBRES_EDIT'))         define('CXX_MEMBRES_EDIT', AXX_ADMIN);
        if (!defined('CXX_MEMBRES_STATS'))        define('CXX_MEMBRES_STATS', AXX_MEMBER);
        if (!defined('CXX_MEMBRES_NEWPASS'))      define('CXX_MEMBRES_NEWPASS', AXX_ROOTADMIN);
        if (!defined('CXX_MEMBRES_DELETE'))       define('CXX_MEMBRES_DELETE', AXX_ROOTADMIN);
        if (!defined('CXX_MEMBRES_ADMIN'))        define('CXX_MEMBRES_ADMIN', AXX_ROOTADMIN);
