...
MSG;
        ...
        $can_search     = (DataEngine::CheckPerms(CXX_CARTE_SEARCH));
...
<div id="Map_Entete" style="width:100%; height:30px; top:50px; position:absolute;">
	<TABLE id="Map_Entete" width={$nav_size}px>
		<TR id="Map_Entete">	
			...
			<td id="Coord">&nbsp;0000&nbsp;</td>	
NAV;
        if (DataEngine::CheckPerms(CXX_CARTE_SEARCH))
            $out.=<<<SEARCH
<form id="searchempire" action="Carte.php" method="post" OnSubmit="return Navigateur.DoSearch();">
			<td {$msg_search1} align=right><input type=radio name="type" value="emp"> Empire <input type=radio name="type" value="jou" checked> Joueur :</td>
			<td {$msg_search2}><input id="Map_Itineraire" type=text name=search value="{$search}"></td>
</form>
SEARCH;
        $out.=<<<f
                        </TR>
