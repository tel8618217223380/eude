...
h;
        $this->PushOutput($out);
        if(DataEngine::CheckPerms(CXX_MEMBRES_NEWPASS))
            $this->PushOutput('<td>nouveau mot de passe</td>');
        if(DataEngine::CheckPerms(CXX_MEMBRES_DELETE))
...
r2;
    ...
    public function footer() {
        $cols = 9;
        if(DataEngine::CheckPerms(CXX_MEMBRES_NEWPASS)) $cols++;
        if(DataEngine::CheckPerms(CXX_MEMBRES_DELETE)) $cols++;
