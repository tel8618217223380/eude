...
DataEngine::CheckPermsOrDie(CXX_MEMBRES_EDIT);
...
if(isset($_POST['ModifJoueur0'])) {
    ...
    */
    ...
    while(isset($_POST['ModifJoueur'.$i])) {
        ...
        if($Joueur['pass'] && DataEngine::CheckPerms(CXX_MEMBRES_NEWPASS)) {
            DataEngine::sql_spool('UPDATE SQL_PREFIX_Users SET Password=md5(\''.$Joueur['pass'].'\') WHERE Login=\''.$Joueur['ID'].'\'');
        }

        if($Joueur['Suppr'] && DataEngine::CheckPerms(CXX_MEMBRES_DELETE)) {
