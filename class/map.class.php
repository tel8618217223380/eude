...
class map /*extends parcours*/ {
    ...
    public function __construct() {
        ...
        $this->cxx_empires = DataEngine::CheckPerms(CXX_CARTE_SHOWEMPIRE);
    ...
    public function load_prefs($value) {
        ...
        $this->pnj			= $tmp[6];
        $this->ennemis		= $tmp[7];
        // TODO [future] Remove loadpref allys upgrading...
        $this->allys		= (isset($tmp[8])) ? $tmp[8]: 1;
    ...
    private function perms_prefs() {
        // TODO force pref from CXX
        if (!DataEngine::CheckPerms(CXX_CARTE_JOUEUR)) $this->joueur = 0;
    ...
    public function init_map($custom='') {
        global $vortex_a, $CurrSS, $CurrSS_a; // Init ici
    ...
    public function add_ss($force_ss=false, $callback=NULL) {
        ...
        if ($this->itineraire && is_array($this->parcours)) { // Mode itinéraire...
            ...
            foreach($this->parcours[1] as $k => $v) {
                // TODO change this
                if ( $CurrSS == $this->Parcours()->get_coords_part($v) ) {
                    switch ($k) {
