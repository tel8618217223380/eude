...
if (!DataEngine::CheckPerms('ZZZ_COMMERCE_MODULES'))
  ...
  $boutonscommande = '<input type="submit" name="submit" value="Suivant &gt;&gt;"> <input type="submit" name="submit" value="Vider panier">';
//  $boutonscommande = '<input type="submit" name="submit" value="Vider panier"> <input type="submit" name="submit" value="Suivant &gt;&gt;">';
  $tnbcol = 6;
...
<?php
  ...
  (SELECT COUNT(SQL_PREFIX_Modules_Users.Login) FROM SQL_PREFIX_Modules_Users LEFT JOIN SQL_PREFIX_Users_Config ON SQL_PREFIX_Modules_Users.Login = SQL_PREFIX_Users_Config.Login WHERE Module_ID = ID AND IF(Categorie=4, ChassisSecret, CommerceType)<=1) AS TotalFab, 
...
//  pDebug($sqlreq);
  ...
  while($ligne = mysql_fetch_array($mysql_result))
    ...
    if($modecommande) {
      if($i == 0 && !$titremod) {
        echo("<tr bgcolor='#000000'><td colspan=".$tnbcol." align=right>".$boutonscommande."</td></tr>");
        $titremod = true;
      }
    ...
    if($oldtitle!=$nowtitle){
      echo("<tr bgcolor='#555555'><td colspan=".$tnbcol." align=center><font size=+1 weight=bold color=yellow>".$nowtitle."</font></td></tr>");
      $oldtitle = $nowtitle;
      $i = 0;
    }
    ...
    if($modecommande) {
      ...
      echo("<input type=text name='Mod_".$ligne["ID"]."' size=5 maxlength=5 value='".$value."'>");
    }
    else {
