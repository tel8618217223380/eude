...
class troops {
    ...
    function AddBattle ($idate, $coords, $def, $att) {
        ...
        $type='attacker';
        if (stripos($def, $_SESSION['_login']) > 0) $type = 'defender';
...
// TODO: revoir le stockage des pids (réelement utile ?)
        $sql = sprintf('INSERT INTO SQL_PREFIX_troops_attack '.
                ...
                $type, $idate, $idsys, $iddet, sqlesc($def), sqlesc($att), sqlesc(serialize(array())) );
        $result = DataEngine::sql($sql);
    ...
    function AddPillage_log ($mode, $idate, $msg) {
        ...
        $fields   = array();
        $sets     = array();
        $fields[] = 'date'; $sets[]= $idate;
        $fields[] = 'mid'; $sets[]= $mid;
        $fields[] = 'Player'; $sets[]= '\''.sqlesc($_SESSION['_login']).'\'';
        foreach ($ares as $k => $v) {
