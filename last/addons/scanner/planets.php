...
$coords = GetSSlistHumainLike($coord, 10);
...
if (preg_last_error()!=PREG_NO_ERROR) die('err preg 1');
...
if (preg_last_error()!=PREG_NO_ERROR) die('err preg 2');
...
//    sleep(rand(5, 8));
    ...
    if (trim($galaxy_info[$i][3]) != '') {
        ...
        if (preg_last_error()!=PREG_NO_ERROR) die('err preg 3');
        $joueur = $player[0][1];
        $empire = $player[0][2];
    } else {
        $joueur = '';
